...
if (isset($_GET['search']) && $_GET['search'] !== '') {
    $search = "%" . $_GET['search'] . "%";
    $stmt = $koneksi->prepare("SELECT * FROM tasks WHERE user_id = ? AND task_name LIKE ? ORDER BY due_date ASC");
...
// Cek dan simpan peringatan
$warnings = [];
while ($task = $tasks->fetch_assoc()) {
    if (!isset($task['due_date'], $task['task_name'], $task['reminder_sent'])) continue;
...
// Karena $tasks udah dipakai di atas (while fetch_assoc), kita perlu ambil ulang
$stmt->execute(); // eksekusi ulang
$tasks = $stmt->get_result(); // ambil ulang result set
?>
...
<form method="GET" style="margin-bottom: 20px;">
    <input type="text" name="search" placeholder="Search task..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
...
<div class="input-container">
        ...
        while ($task = $tasks->fetch_assoc()) {
            ...
            echo "<form method='POST' style='display:inline;' onsubmit=\"return confirm('Apakah kamu yakin ingin menghapus tugas ini?');\">
